    ...
    public class InventoryRenderer : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            _imagePool = new Pool<Image>(
                delegate
                {
                    ...
                    image.name = "image" + itemsHolder.items.Count;
                ...
                default:
                    ...
                    for (int y = 0; y < inventory.height; y++)
                    {
                        for (int x = 0; x < inventory.width; x++)
                        {
                            ...
                            grid.gameObject.tag = "slot";
                            var col = grid.gameObject.AddComponent<BoxCollider2D>();
                            col.size = grid.rectTransform.sizeDelta;
                            col.isTrigger = true;
                            if(grid.gameObject.GetComponent<Rigidbody2D>() == null)
                            {
        ...
        */
        private void HandleItemAdded(IInventoryItem item)
        {
            ...
            if (img.gameObject.GetComponent<BoxCollider2D>() == null)
            {
                var col = img.gameObject.AddComponent<BoxCollider2D>();
                col.size = img.rectTransform.sizeDelta;
            }
            if (img.gameObject.GetComponent<Rigidbody2D>() == null)
            {
                var rb = img.gameObject.AddComponent<Rigidbody2D>();
                rb.isKinematic = true;
            }
